...
%Random generation of height of the core(m)
hmin = 2e-2;
hmax = 5e-2;
hop = (hmin - hmax)*rand(1,N_ex) + hmax;

%Random generation of widht of the core(m)
wmin = 2e-2;
wmax = 5e-2;
wop = (wmin - wmax)*rand(1,N_ex)  + wmax;

%Random generation of cross-sectional area of the core(m^2)
Acmin = 0.25e-4;
Acmax = 0.6e-4;
...
%Random generation of area to fit Inductor(m^2)
Amin = 4e-4;
Amax = 30.25e-4;
Aip = (Amin - Amax)*rand(1,N_ex)  + Amax;

%Random generation of the air gap length(m)
lgmin = 1e-3;
lgmax = 5e-3;
